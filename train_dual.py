...
def collect_fn(features: dict):
    to_inp = {"code": [f['code'] for f in features],
              ...
              "code_score": [f['code_score'] for f in features],
              "summ_score": [f['summ_score'] for f in features]}
    return to_inp
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_dual",
                        type=float,
                        default=0.01,
                        help="Hyper-parameters to control cs_dual_loss")
    parser.add_argument("--lambda_attn",
                        type=float,
                        default=0.01,
                        help="Hyper-parameters to control cs_attn_loss")  

    parser.add_argument("--train_code_path",
                        default="data/java/train/code.original")
    parser.add_argument("--train_code_score_path",
                        default="data/java/train/code.score")
    parser.add_argument("--train_summ_path",
                        default="data/java/train/javadoc.original")
    parser.add_argument("--train_summ_score_path",
                        default="data/java/train/javadoc.score")
    ...
    dual_model = DualModel(config=cs_model.config,
                           cs_model=cs_model,
                           cg_model=cg_model,
    ...
    train_dataset = create_dataset(code_fp=args.train_code_path,
                                   ...
                                   code_score_fp=args.train_code_score_path,
                                   summ_score_fp=args.train_summ_score_path)
