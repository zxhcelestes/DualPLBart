...
parser.add_argument('--data',
                    type=str,
                    default='../data/code_replaced.txt',
...
model = model.RNNModel(ntokens, args.emsize, args.nhid, args.nlayers,
                       args.dropout, args.tied)
if os.path.exists(args.save):
    try:
        print("find a trained model, try to load it")
        model.load_state_dict(torch.load(args.save))
        print("successful")
    except Exception:
        print("load error, init a new one")
if args.cuda:
    model.cuda()
...
model = model.RNNModel(
    ntokens, args.emsize, args.nhid, args.nlayers, args.dropout,
    args.tied)  # map_location=lambda storage, loc: storage)
model.load_state_dict(torch.load(args.save))
if args.cuda:
    model = model.cuda()
