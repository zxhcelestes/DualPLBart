...
class Corpus(object):
    def __init__(self, path):
        ...
        if os.path.exists(os.path.join(path, 'dict_code.pkl')):
            print("find a dict")
            with open(os.path.join(path, 'dict_code.pkl'), 'rb') as f:
                self.dictionary = pickle.load(f)
        self.train = self.tokenize(os.path.join(path, 'train.token.nl'))
